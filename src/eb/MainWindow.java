...
public class MainWindow extends JFrame implements DeckChangeListener {
	...
	/**
	 * [CPPRCCC] Returns the commands of the user interface as a string, which
	 * can be used for example to instruct the user on Eb's use.
	 * 
	...
	private String getUICommands() {
		// preconditions: none
		// postconditions: none
		return "Ctrl+N to add a card. Ctrl+Q to quit. " + "Ctrl+T to view/edit the study options.";
	}
	...
	MainWindow() {
		...
		studyOptionsItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_T, ActionEvent.CTRL_MASK));
