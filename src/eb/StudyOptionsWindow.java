...
public class StudyOptionsWindow extends JFrame {
  ...
  // [CCCC] Input element that allows users to view and set the interval between
  // the creation of the card and the first time it is studied.
  ...
  // [CCCC] Button that restores the defaults to those of Eb 
  private JButton m_loadEbDefaultsButton;
  
  // [CCCC] Button that reloads the current settings of the deck (undoing non-saved
  // changes)
  private JButton m_loadCurrentDeckSettingsButton;
  
  // [CCCC] Button that sets the study settings of this deck to the current values
  private JButton m_setToTheseValuesButton;
  
  /** @@@[C
   * A TimeInputElement contains a textfield and combobox that allow the user
   * to input say "5.5 minutes" or "3 hours".
   * 
   * @author Eric-Wubbo Lameijer
   */
  class TimeInputElement extends JPanel {
  	
    // [CCCC] Textfield that sets the quantity (the '3' in 3
    // hours), the combobox selects the unit (the hours in "3 hours").
     ...
     * time interval.
     * @param timeInterval the time interval that should be displayed by this
    ...
    public void setInterval(TimeInterval timeInterval) {
    	...
    	Utilities.require( timeInterval != null, 
    			...
    			"may not be null.");
    	m_scalarField.setText(Utilities.doubleToMaxPrecisionString(
    			timeInterval.getScalar(),2));
    	m_unitComboBox.setSelectedItem(
    ...
    public TimeInputElement(TimeInterval timeInterval) {
      ...
      m_unitComboBox.setModel(new DefaultComboBoxModel<String>(
      		TimeUnit.getUnitNames()));
      add(m_unitComboBox);
   ...
   * NOTE: EVIL DUPLICATION. CAN I AVOID THAT?
   */
  private void close() {
    // preconditions: none
    this.dispose();
  ...
  public StudyOptionsWindow() {
    ...
    m_loadCurrentDeckSettingsButton.addActionListener(
    		e -> loadCurrentDeckSettings());
    m_setToTheseValuesButton = new JButton("Set study parameters of this " +
    	"deck to these values");
