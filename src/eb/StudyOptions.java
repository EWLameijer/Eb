...
public class StudyOptions implements Serializable {
	...
	// [CCCC] The serialization ID. Automatically generated, can be ignored
	private static final long serialVersionUID = -5967297039338080285L;
	
	// [CCCC] The interval between creation of a card and when it should first
	// be reviewed. Can be set by the user.
	private TimeInterval m_initialInterval;
	
	// [CCCC] The default initial interval.
	private final static TimeInterval DEFAULT_INITIAL_INTERVAL = 
			new TimeInterval(10.0, TimeUnit.MINUTE);
	
	/** @@@[CPPRCC
	 *  Returns the interval that a newly created card has to wait before it 
	 *  should be reviewed (after all, reviewing a card 2 seconds after making it
	 *  probably won't teach you much, as it is still too fresh in memory.
	 *  
