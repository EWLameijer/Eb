 ...
 * subject, like Spanish) that can be studied by the user. As there is only one
 * deck active in Eb, and it needs to be shared with all UI windows, a singleton
...
public class Deck {
  ...
  /**
   * [CPPRCCC] Returns whether a deck / the contents of a deck have been loaded.
   * 
  ...
  /**
   * [CPPRCCC] Returns whether the deck has been initialized, even if it is only
  ...
  /**
   * [CPPRCCC] Loads a deck from file.
   * 
   * @param name
  ...
  /**
   * [CPPRCCC] Creates a deck with name "name".
   * 
   * @param name
  ...
  /**
   * [CPPRCCC] Saves the deck to disk.
   */
  ...
  /**
   * [CPPRCCC] Adds a card to the deck. Should only be called after the
   * card-adding validity is ensured using 'canAddCard' - if that is erroneously
   * forgotten, this function will crash hard to prevent worse programming
   * problems to crop up later.
   * 
   * @param card
   ...
   * in the deck, and the back is not a null pointer. Note: delegates call to
   * DeckContents.
   * 
   * @param card
