...
/**
 * [CC]
 * Contains the properties belonging to the 'pure' deck itself, like its
...
public class DeckContents implements Serializable {
  ...
  /**
   * [CPPRCCC] Returns the File object representing a deck with name
   * "deckName" on disk.
   * 
  ...
  protected File getFileHandle() {
	...
	Utilities.require(deckFileHandle != null,
		"Deck.getFileHandle error: " + "problem creating file handle for deck.");
  ...
  public int getCardCount() {
	// preconditions
	Utilities.require(m_cards != null, "DeckContents.getCardCount error: Cards not initialized!");
  ...
  /** [CPPRCCC]
   * Returns whether this new front (text) does not already occur in the 
   * deck - after all, the deck is a kind of map, with a stimulus 
   * (front) leading to only one response (back).
   * @param front the front of the card to be checked for uniqueness in the 
   * deck
   * @return whether the card would be a valid addition to the deck (true) or
   * whether the front would be a duplicate of a front already present (false)
   */
  ...
  protected boolean canAddCard(Card card) {
	...
	Utilities.require( card != null, "Deck.canAddCard error: " + 
  ...
  protected void addCard(Card card) {
	Utilities.require( canAddCard( card), 
		...
		"that is intended to be added is invalid. The 'canAddCard' method " + 
		"has to be invoked first to check the possibility of the current " + 
		"method.");
